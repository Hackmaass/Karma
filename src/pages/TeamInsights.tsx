...
function FlowItem({ from, to, strength, alert }: FlowItemProps) {
  return (
...
function BottleneckItem({ icon, title, desc, impact }: BottleneckItemProps) {
  return (
    <div className="flex items-start gap-4">
      ...
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full ${impact === 'High' ? 'bg-amber-100 text-amber-800' : 'bg-black/[0.04] text-black/60'
            }`}>
            {impact} Impact
          </span>
        </div>
